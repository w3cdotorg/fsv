...
| 2026-08-21 | Scan cache (1999 TODO): replay is per-directory on `mtime`+`ctime` equality of a *fresh* lstat vs the snapshot -- files inside a matching directory skip lstat entirely, subdirectories are always lstat'ed fresh | a directory's mtime covers only its own entry set, never its descendants, so subtree-skipping on the root's mtime alone would be wrong; files are the overwhelming majority of nodes, so skipping their lstats (plus scandir on unchanged dirs) is where the 15x cached-startup win on /opt/homebrew (406k nodes, 9.3s -> 0.6s) actually comes from |
| 2026-08-21 | Scan cache: in-place file edits inside an unchanged directory stay stale until the directory changes or File->Rescan skips the cache (`fscache_skip_next_load()`) -- locked in by a test that asserts the staleness | this is the 1999 ticket's own design trade-off; asserting it in `tests/test_fscache.c` makes any future semantic change a deliberate act instead of an accident, and Rescan bounds the damage to one explicit click |
| 2026-08-21 | Scan cache: native-endian versioned binary under `g_get_user_cache_dir()/fsv/` with the ticket's own `@usr@lib` name encoding, exclusion state fingerprinted in the header, atomic tmp+rename write, every mismatch degrading to a full scan | a cache is a per-machine artifact -- discarding it is always correct, so the format optimizes for cheap bounds-checked parsing (names point straight into the loaded image) rather than portability; the fingerprint prevents a cache written under one `--exclude` regime from replaying under another |
| 2026-08-21 | Scan cache: `fscache_prev_scan_time()`/`replayed_dirs` deliberately survive `fscache_release()` | they describe the scan that just consumed the index; the Color Setup "Since previous scan" preset (the ticket's "graphical diff" half) and the tests both read them after `scanfs()` has already dropped the index |
| 2026-08-21 | `tests/test_fscache.c` backdates every fixture directory's mtime by 100s after setup, and holds `xstrdup(xgetcwd())` rather than the raw pointer | mtimes are second-granular and the loader's race guard distrusts snapshot-second mtimes, so un-backdated fixtures are sleep()-flaky; and `xgetcwd()` returns its own static, realloc-on-call buffer, which later `scanfs()` calls move -- the raw pointer was a use-after-free that made the test fail intermittently until caught |
| 2026-08-21 | Sub-pixel pick promotion (1999 TODO "smarter pointing"): `gpu_pick_window()` reads the 17x17 id census around the cursor from the same render `gpu_pick()` already did, and `node_at_cursor()` walks the hit up to the first ancestor whose subtree covers >= 16 px of it | the census reuses data the pick render already produced (one slightly larger readback, no second render, no per-mode projected-size math -- the pick buffer is mode-agnostic by construction); 16 px is a ~4x4 target: big enough that a deliberate click on a visible node saturates it and resolves unchanged, verified with a posed birds-eye dolly-out harness (4-px specks promote to their pedestal, a 64-px box stays itself) |
| 2026-08-21 | The macOS GTK-frontend blocker was closed by deleting `src/ogl.c`'s `#include <GL/glu.h>` and the darwin gate in `src/meson.build`, not by the "future OpenGL-removal task" the old note reserved it for | the include (annotated `gluPickMatrix( )`) was a leftover: `grep 'glu[A-Z]' src/ogl.c` finds no call -- the modern select path never used a pick matrix. Verified by a real macOS build+launch (Homebrew gtk+3/epoxy): the GL realize path runs (glyph atlas logged from inside ogl.c's GL init). Found in the process, filed in TODO.md: `fsv ROOTDIR` trips a GLib-GIO CRITICAL in the GTK arm on every platform (argv handed whole to `g_application_run()` without `G_APPLICATION_HANDLES_OPEN`) |
| 2026-08-21 | DiscV's scrollbar pair implemented as mapv's structural mirror, with the disc-center ancestor sum inlined in camera.c instead of calling `geometry_discv_node_pos()` | that accessor lives in the frontends' GL-bound geometry.c; calling it from libfsvcore would hand headless camera tests a stub instead of the real walk. Also corrected mid-task: `camera_scrollbar_moved()` already recenters the slider value (+0.5 page) before dispatching, so the move side takes the value raw (x) / negated (y) -- an earlier draft double-added the offset and the round-trip test caught it |
