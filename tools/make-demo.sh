...
#
# Builds docs/media/demo.mp4 + docs/media/demo.gif: Task 6.4's ≤20s demo
# video of fsv navigating this repository's own src/ tree.
#
# Pipeline:
#   1. Build the SDL/Metal frontend (builddir, meson's default target
#      name -- see README.md/docs/PORTING.md) if it doesn't exist yet.
#   2. `fsv --record`: a scripted camera flythrough of src/, rendered
#      offscreen (scene + ImGui composited, see src/sdl/gpu.cpp's
#      "--record" section) into numbered BMP frames in a scratch dir.
...
#   duration_seconds defaults to 19 (Task 6.4's ≤20s budget, with a
#   1s margin below the hard cap).
...
"$FSV_BIN" src --record "$FRAMES_DIR" "$DURATION"
