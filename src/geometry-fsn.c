 ...
 *
 * This is the LAYOUT half, and it is pure math: it reads the scanned
 * tree and writes per-node geometry into the node descriptors' own
 * scratch space, with no renderer dependency of any kind. That is what
 * puts it in libfsvcore next to camera.c (which reads the layout to
 * frame the landscape) and what lets tests/test_fsn_layout.c exercise it
 * headless, with no gpu stubs at all.
 *
 * The DRAW half -- everything that touches gpu.h -- is
 * src/geometry-fsn-draw.c, which is frontend-side like geometry.c.
 *
